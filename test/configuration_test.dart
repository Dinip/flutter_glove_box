...
void main() {
  group('GoldenToolkitConfiguration Tests', () {
    ...
    group('Shadows', () {
      GoldenToolkit.runWithConfiguration(
        () {
          testGoldens(
            ...
            await screenMatchesGolden(tester, 'enableRealShadows_honored_when_testGoldens_wrapped');

            expect(debugDisableShadows, isFalse, reason: 'debugDisableShadows should be false during this test');
          });
        },
      ...
      testGoldens(
        ...
        await GoldenToolkit.runWithConfiguration(
          () async {
            await tester.pumpWidgetBuilder(const WidgetWithShadows());
            await screenMatchesGolden(tester, 'enableRealShadows_ignored_when_testGoldens_not_wrapped');

            expect(debugDisableShadows, isTrue, reason: 'debugDisableShadows should be true during this test');
          },
          config: GoldenToolkit.configuration.copyWith(enableRealShadows: true),
        );
      });
    ...
    group('Tags', () {
      testGoldens('multiScreenGolden method uses tags from global configuration', (tester) async {
        expect(Invoker.current!.liveTest.test.metadata.tags, const ['golden']);
      });

      testGoldens('multiScreenGolden method uses overridden tags', (tester) async {
    ...
    group('Equality/Hashcode/CopyWith', () {
      ...
      group('should not be equal when params differ', () {
        ...
        test('enableRealShadows', () {
          expect(config, isNot(equals(config.copyWith(enableRealShadows: !enableRealShadows))));
          expect(config.hashCode, isNot(equals(config.copyWith(enableRealShadows: !enableRealShadows).hashCode)));
        });

        test('tags', () {
          expect(config, isNot(equals(config.copyWith(tags: 'foo'))));
          expect(config.hashCode, isNot(equals(config.copyWith(tags: 'foo').hashCode)));
        });
      });
    });
  });
