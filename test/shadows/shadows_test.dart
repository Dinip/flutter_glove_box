...
void main() {
  group('Shadows Tests', () {
    setUp(() {
      expect(debugDisableShadows, isTrue, reason: 'debugDisableShadows should always be true before test');
    });

    tearDown(() {
      expect(debugDisableShadows, isTrue, reason: 'debugDisableShadows should always be true after test');
    });
    ...
    testGoldens(
      ...
      expect(debugDisableShadows, isFalse, reason: 'debugDisableShadows should be false during test');
    });

    // wraping needs to happen outside of testGoldens
    GoldenToolkit.runWithConfiguration(
      () => {
        testGoldens('screenMatchesGolden method can disable enableRealShadows temporarily', (tester) async {
          await tester.pumpWidgetBuilder(const WidgetWithShadows());
          await screenMatchesGolden(tester, 'realShadows_disabled_locally');

          expect(debugDisableShadows, isTrue, reason: 'debugDisableShadows should be false during test');
        })
      },
